...
function debug:on_key_pressed(key, modifiers)
  ...
  if key == "1" then
    if sol.game.exists("save1.dat") then
      game_manager:create("save1.dat"):start()
    end
  elseif key == "2" then
    if sol.game.exists("save2.dat") then
      game_manager:create("save2.dat"):start()
    end
  elseif key == "3" then
    if sol.game.exists("save3.dat") then
      game_manager:create("save3.dat"):start()
    end
  elseif key == "f12" and not console.enabled then
    sol.menu.start(sol.main, console)
  elseif sol.main.get_game() ~= nil and not console.enabled then
